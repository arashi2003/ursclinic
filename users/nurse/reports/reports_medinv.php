<?php
    ...
    $query = mysqli_query($conn, "SELECT * FROM report_medsupinv $ca $date AND type != 'supply' ORDER BY admin");
    while($data=mysqli_fetch_array($query))
    {
        if($data['type'] == "med_admin")
        {
            $pdf->SetFont('Arial','B',8);
            $pdf->Cell(85, 5, $data['medicine'] . ' MEDICINE', 1, 0,'C');
