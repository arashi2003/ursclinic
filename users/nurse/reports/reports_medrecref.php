<?php
    ...
    $query = mysqli_query($conn, "SELECT firstname, middlename, lastname, account.campus, age, designation, sex, birthday, department, college, program, yearlevel, section, chiefcomplaint, findings, issued, referral, date, time FROM account INNER JOIN patient_info pi ON pi.patientid=accountid INNER JOIN  med_history mh on mh.userid=accountid WHERE accountid = '$account_no'");
    ...
    if($data=mysqli_fetch_array($query))
    {
        ...
        $datetime = date("F d, Y g:i A", strtotime($dt));
        $designation = $data['designation'];
        $cc = $data['chiefcomplaint'];
        $findig = $data['findings'];
        $issued = $data['issued'];
        $ref = $data['referral'];
        ...
        $pdf->Cell(1, 0, $course . " " . $year . "-". $section, 0);
        $pdf->SetFont('Arial', '', 10);
        ...
        $pdf->Cell(1, 4.5, '_________________________________________________________________', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell(127.5, 4, $findig, 'B,B', 0);
        $pdf->SetFont('Arial', '', 10);
        ...
        $pdf->Cell(1, 4.5, '_________________________________________________________________', 0);
        $pdf->SetFont('Arial', '', 8);
        $pdf->MultiCell(127.5, 4, $issued, 'B,B', 0);
        $pdf->SetFont('Arial', '', 9);
        ...
        $pdf->SetFont('Arial', '', 9);
        $pdf->MultiCell(127.5, 4.5, $ref, 'B,B', 0);
        $pdf->SetFont('Arial', '', 10);
    }
