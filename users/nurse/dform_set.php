...
if (isset($_GET['dform_set'])) {
    // Validate and sanitize input
    $dform_set = isset($_GET['dform_set']) ? $_GET['dform_set'] : '';
    ...
    if ($dform_set !== '') {
        $whereClause .= " WHERE dosage_form LIKE '%$dform_set%'";
    }

    // Construct and execute SQL query for counting total rows
    $sql_count = "SELECT COUNT(*) AS total_rows FROM dosage_form $whereClause ORDER BY dosage_form";
} else {
    // If filters are not set, count all rows
    $sql_count = "SELECT COUNT(*) AS total_rows FROM dosage_form ORDER BY dosage_form";
}
                ...
                <div class="inv-tabs">
                    <div class="tabs">
                        <ul class="nav nav-pills">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" aria-current="page" href="#">Dosage Form</a>
                            </li>
                            <li class="nav-item">
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        if (isset($_GET['dform_set']) && $_GET['dform_set'] != '') {
                                            ...
                                            $count = 1;
                                            $sql = "SELECT * FROM dosage_form WHERE dosage_form LIKE '%$dform_set%' ORDER BY dosage_form LIMIT $start, $rows_per_page";
                                            $result = mysqli_query($conn, $sql);
                                        } else {
                                            $count = 1;
                                            ...
                                            if (mysqli_num_rows($result) > 0) {
                                                foreach ($result as $data) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#updatedform_set<?php echo $data['id']; ?>">Update</button>
                                                            <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#removedform_set<?php echo $data['id']; ?>">Remove</button>
                                                        </td>
                                                    </tr>
                                                <?php
                                ...
                                <ul class="pagination justify-content-end">
                                    ...
                                    if (mysqli_num_rows($result) > 0) : ?>
                                        <li class="page-item <?= $page == 1 ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?<?= isset($_GET['dform_set']) ? 'dform_set=' . $_GET['dform_set'] . '&' : '' ?>page=<?= 1; ?>">&laquo;</a>
                                        </li>
                                        <li class="page-item <?php echo $page == 1 ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?<?= isset($_GET['dform_set']) ? 'dform_set=' . $_GET['dform_set'] . '&' : '' ?>page=<?= $previous; ?>">&lt;</a>
                                        </li>
                                        <?php for ($i = $start_loop; $i <= $end_loop; $i++) : ?>
                                            <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>">
                                                <a class="page-link" href="?<?= isset($_GET['dform_set']) ? 'dform_set=' . $_GET['dform_set'] . '&' : '' ?>page=<?= $i; ?>"><?= $i; ?></a>
                                            </li>
                                        <?php endfor; ?>
                                        <li class="page-item <?php echo $page == $pages ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?<?= isset($_GET['dform_set']) ? 'dform_set=' . $_GET['dform_set'] . '&' : '' ?>page=<?= $next; ?>">&gt;</a>
                                        </li>
                                        <li class="page-item <?php echo $page == $pages ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?<?= isset($_GET['dform_set']) ? 'dform_set=' . $_GET['dform_set'] . '&' : '' ?>page=<?= $pages; ?>">&raquo;</a>
                                        </li>
                                    <?php endif; ?>
                                </ul>
                            </div>
