...
if (isset($_GET['transaction'])) {
    ...
    if ($transaction !== '') {
        $whereClause .= " WHERE service LIKE '%$transaction%'";
    }

    // Construct and execute SQL query for counting total rows
    $sql_count = "SELECT COUNT(*) AS total_rows FROM transaction $whereClause ORDER BY transaction_type";
} else {
    // If filters are not set, count all rows
    $sql_count = "SELECT COUNT(*) AS total_rows FROM transaction ORDER BY transaction_type";
}
                ...
                <div class="inv-tabs">
                    <div class="tabs">
                        <ul class="nav nav-pills">
                            <li class="nav-item">
                                <a class="nav-link" aria-current="page" href="#">Chief Complaint</a>
                            </li>
                            <li class="nav-item">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" aria-current="page" href="#">Transaction/Service</a>
                            </li>
                        </ul>
                    </div>
                    <div>
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addtransaction_set">Add Transaction/Service</button>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12 mb-2">
                                            <div class="input-group">
                                                <input type="text" name="transaction" value="<?= isset($_GET['transaction']) == true ? $_GET['transaction'] : '' ?>" class="form-control" placeholder="Search chief complaint">
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        ...
                                        if (isset($_GET['transaction']) && $_GET['transaction'] != '') {
                                            ...
                                            $count = 1;
                                            $sql = "SELECT * FROM transaction WHERE service LIKE '%$transaction%' ORDER BY transaction_type LIMIT $start, $rows_per_page";
                                            $result = mysqli_query($conn, $sql);
                                        } else {
                                            $count = 1;
                                            $sql = "SELECT * FROM transaction ORDER BY transaction_type LIMIT $start, $rows_per_page";
                                            $result = mysqli_query($conn, $sql);
                                        }
                                        if ($result) {
                                ...
                                <ul class="pagination justify-content-end">
                                    ...
                                    if (mysqli_num_rows($result) > 0) : ?>
                                        <li class="page-item <?= $page == 1 ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?<?= isset($_GET['transaction']) ? 'transaction=' . $_GET['transaction'] . '&' : '' ?>page=<?= 1; ?>">&laquo;</a>
                                        </li>
                                        <li class="page-item <?php echo $page == 1 ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?<?= isset($_GET['transaction']) ? 'transaction=' . $_GET['transaction'] . '&' : '' ?>page=<?= $previous; ?>">&lt;</a>
                                        </li>
                                        <?php for ($i = $start_loop; $i <= $end_loop; $i++) : ?>
                                            <li class="page-item <?php echo $page == $i ? 'active' : ''; ?>">
                                                <a class="page-link" href="?<?= isset($_GET['transaction']) ? 'transaction=' . $_GET['transaction'] . '&' : '' ?>page=<?= $i; ?>"><?= $i; ?></a>
                                            </li>
                                        <?php endfor; ?>
                                        <li class="page-item <?php echo $page == $pages ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?<?= isset($_GET['transaction']) ? 'transaction=' . $_GET['transaction'] . '&' : '' ?>page=<?= $next; ?>">&gt;</a>
                                        </li>
                                        <li class="page-item <?php echo $page == $pages ? 'disabled' : ''; ?>">
                                            <a class="page-link" href="?<?= isset($_GET['transaction']) ? 'transaction=' . $_GET['transaction'] . '&' : '' ?>page=<?= $pages; ?>">&raquo;</a>
                                        </li>
                                    <?php endif; ?>
                                </ul>
                            </div>
